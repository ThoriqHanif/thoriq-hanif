...
const Facemesh = /*#__PURE__*/React.forwardRef(({
  ...
  const geometry = React.useMemo(() => {
    ...
    const arr = [];
    face.keypoints.forEach(({
      x,
      y,
      z
    }) => arr.push(x, y, z));
    const vertices = new Float32Array(arr);
    geometry.setAttribute('position', new THREE.BufferAttribute(vertices, 3)); // Indices

    geometry.setIndex(TRIANGULATION); // center

    geometry.translate(-face.box.xMin - face.box.width / 2, -face.box.yMin - face.box.height / 2, 0); // re-scale

    let scale;
    if (width) scale = width * 1 / face.box.width; // fit in width

    if (height) scale = height * 1 / face.box.height; // fit in height

    geometry.scale(scale, scale, scale); // flipY
...
export { Facemesh, MYFACE, TRIANGULATION };
